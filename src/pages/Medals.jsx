      ...
      <div className="page-body">
        {loading ? <div className="loading">LOADING...</div> : (
          <>
            ...
            {tab === 'medals' && (
              <>
                <div style={{ fontSize: 10, letterSpacing: '.15em', color: 'var(--accent)', fontFamily: 'var(--font-mono)', marginBottom: 16 }}>◆ ALL COMMENDATION PATCHES ({medals.length})</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 16, marginBottom: 32 }}>
                  {medals.map(m => (
                    <div key={m.id} className="card" style={{ textAlign: 'center', padding: '20px 12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                      <MedalPatch name={m.name} rarity={m.rarity} size={90} />
                      <div style={{ fontWeight: 600, fontSize: 12, letterSpacing: '.04em' }}>{m.name}</div>
                      <span className={`badge ${RARITY_BADGE[m.rarity]}`} style={{ fontSize: 9 }}>{m.rarity}</span>
                      <span className="badge badge-muted" style={{ fontSize: 8 }}>{m.category}</span>
                      {m.description && <div style={{ fontSize: 10, color: 'var(--text-3)', lineHeight: 1.5 }}>{m.description}</div>}
                    </div>
                  ))}
                </div>
                ...
                {memberMedals.length === 0 ? <div className="empty-state">NO AWARDS YET</div> : (
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {memberMedals.slice(0, 20).map(mm => (
                      <div key={mm.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {mm.reason && <div style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{mm.reason}</div>}
                        </div>
                  ...
                  {certs.map(c => {
                    ...
                    return (
                      <div key={c.id} className="card">
                        ...
                        {c.description && <div style={{ fontSize: 11, color: 'var(--text-2)', marginBottom: 6, lineHeight: 1.5 }}>{c.description}</div>}
                        <div style={{ fontSize: 10, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>{holders} certified member{holders !== 1 ? 's' : ''}</div>
                      </div>
                    )
                  })}
                </div>
            ...
            {tab === 'mine' && (
              <>
                <div style={{ fontSize: 10, letterSpacing: '.15em', color: 'var(--accent)', fontFamily: 'var(--font-mono)', marginBottom: 16 }}>◆ MY MEDALS ({myMedals.length})</div>
                {myMedals.length === 0 ? <div className="empty-state" style={{ padding: '20px 0' }}>NO MEDALS EARNED YET</div> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 14, marginBottom: 32 }}>
                    {myMedals.map(mm => (
                      <div key={mm.id} className="card" style={{ textAlign: 'center', padding: '16px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                        <MedalPatch name={mm.medal?.name} rarity={mm.medal?.rarity} size={72} />
                        <div style={{ fontSize: 11, fontWeight: 600 }}>{mm.medal?.name}</div>
                ...
                {myCerts.length === 0 ? <div className="empty-state" style={{ padding: '20px 0' }}>NO CERTIFICATIONS YET</div> : (
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    {myCerts.map(mc => (
                      <div key={mc.id} className="card" style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span className={`badge ${CERT_CAT_BADGE[mc.cert?.category]}`} style={{ fontSize: 9 }}>{mc.cert?.category}</span>
